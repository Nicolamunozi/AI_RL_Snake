...
from config import REWARD_FOOD, REWARD_DEATH
...
class ScoreBoard(Turtle):
    def __init__(self):
        ...
        # Separate turtle writers so scoreboard, status bar, and menu
        # can be cleared independently without wiping each other.
    ...
    def update_status(self, n_games=0, epsilon=0.0, mode='TRAIN',
                      mean_score=0.0, loaded=False):
        """Show training stats below the score header."""
        self.status_writer.clear()
        self.status_writer.goto(0, 185)
        loaded_text = 'Yes' if loaded else 'No'
        self.status_writer.write(
            f'Games: {n_games} | Mean: {mean_score:.2f} | '
            f'Epsilon: {epsilon:.3f} | Mode: {mode} | Loaded: {loaded_text}',
            align=ALIGNMENT,
            font=SMALL_FONT,
        )
    ...
    def set_step_reward(self):
        """Reset reward to 0 at the start of each step."""
        self.reward = 0
    ...
    def show_menu(self, lines):
        """Render a multi-line start menu centred on screen."""
